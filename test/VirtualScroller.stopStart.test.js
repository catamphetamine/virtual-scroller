...
describe('VirtualScroller', function() {
	it('should show and hide items on scroll (after being restarted)', async function() {
		...
		// Artificially re-start the `VirtualScroller`.
		// https://gitlab.com/catamphetamine/virtual-scroller/-/issues/21
	...
	it('should not allow starting when already started', function() {
		const virtualScroller = new VirtualScroller({
			items: [{ area: 8000 }],
			screenWidth: 800,
			screenHeight: 400,
		...
		virtualScroller.start()
		expect(() => virtualScroller.start()).to.throw('has already been started')
	})

	it('should not allow stopping when already stopped', function() {
		const virtualScroller = new VirtualScroller({
			items: [{ area: 8000 }],
			screenWidth: 800,
			screenHeight: 400,
		...
		expect(() => virtualScroller.stop()).to.throw('hasn\'t been started')
		virtualScroller.start()
		virtualScroller.stop()
		expect(() => virtualScroller.stop()).to.throw('hasn\'t been started')
	})
