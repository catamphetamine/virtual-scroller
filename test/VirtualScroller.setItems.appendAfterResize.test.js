...
describe('VirtualScroller', function() {
	it('should append items after window resize', async function() {
		...
		// Append items.
		const APPENDED_ITEMS_COUNT = PREV_COLUMNS_COUNT
		items = items.concat(new Array(APPENDED_ITEMS_COUNT).fill({ area: ITEM_WIDTH * ITEM_HEIGHT }))
		...
		virtualScroller.expectStateUpdate({
			...
			itemStates: new Array(items.length),
			firstShownItemIndex: 2 * PREV_COLUMNS_COUNT - PREV_COLUMNS_COUNT,
			lastShownItemIndex: 5 * PREV_COLUMNS_COUNT - 1,
			...
			afterItemsHeight: (PREV_ROWS_COUNT - 5) * (PREV_ITEM_HEIGHT + VERTICAL_SPACING) +
				(APPENDED_ITEMS_COUNT / COLUMNS_COUNT) * (PREV_ITEM_HEIGHT + VERTICAL_SPACING)
		})
